...
void ofApp::setup(){
    ...
    FireFlies04 tempP = FireFlies04();  //create a local BasicParticle to pass into our vector which will store BasicParticle objects
    
    particles.push_back(tempP);   //add a new BasicParticle to the rear of our vector
    
    //initializing our forces
    //wind = ofPoint(-0.1,3.2);
    wind = ofPoint(ofRandom(-0.5,3),ofRandom(-0.5, 3));
    gravity = ofPoint(-0.4,0.0);
   // gravity = ofPoint(ofRandom(-0.3, 1));
