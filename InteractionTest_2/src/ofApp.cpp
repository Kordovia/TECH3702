...
void ofApp::setup(){
    ...
    maxBlobs = 2;   //maximum number of blobs and size of our ofPoint vector
    
    blobCenters.resize(maxBlobs);   //set the size or our vector based off the numbe of blobs
    
    f = FireFliesTest2();
    //numFireFliesTest = 10;
    numFireFliesTest2 = 100;
    
    for(int i = 0; i <numFireFliesTest2; i++){
        FireFliesTest2 tempP = FireFliesTest2();
        firefliestest2.push_back(tempP);
    }
...
void ofApp::update(){
    ...
    for(int i = 0; i < contourFinder.nBlobs; i++){
        ...
        for(int i = 0; i <firefliestest2.size(); i++) {
            firefliestest2[i].seek();
            firefliestest2[i].update();
        }
...
void ofApp::draw(){
    ...
    */
    ...
    for(int i = 0; i < firefliestest2.size(); i++) {
        firefliestest2[i].moveTarget(loc);
    }
    
    
    
    for(int i = 0; i < firefliestest2.size(); i++) {
        firefliestest2[i].display();
    }
