...
Fireflies04::Fireflies04(){
    location = ofPoint(ofRandom(ofGetWidth()),1000);    //generate a random location to start from within the sketch window
    ...
    maxSpeed = 2; //setting the initial maximum speed
    
    maxForce = 0.1; //setting the maximum steering force to something small
    
    radius = 15;    //radius of our vehicle shape
}
...
void Fireflies04::display(){
    ...
    ofSetColor(245, 241, 30, v);                  //set the background to the mapped value of v
...
void Fireflies04::moveTarget(ofPoint inTarget){
